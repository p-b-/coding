package main // https://www.hackerrank.com/contests/w26/challenges/game-with-cells
import (
	"bufio"
	"fmt"
	"log"
...
func runTest(m int, n int, debug bool) int {
	if debug {
		fmt.Printf("%d * %d\n", m, n)
	}
	if m == 0 || n == 0 {
...
func main() {
	filenames := []string{"T1.txt", "T2.txt", "T3.txt", "T4.txt", "T5.txt", "T6.txt", "T7.txt", "T8.txt"}
	var start time.Time
